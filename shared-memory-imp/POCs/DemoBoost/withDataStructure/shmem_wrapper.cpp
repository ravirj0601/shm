...
void SMemory::createQueue(std::string name, unsigned int data_size)
{
    try
    {
        if (checkMemory(name))
        ...
        std::cout << "Trying to create QueueSHM: " << std::endl;
        // Increase data_size to a larger value for testing
        ipc::managed_shared_memory managed_shm{ipc::create_only, name.c_str(), 1024 * 1024}; // Allocate 1 MB for testing
        std::cout << "Created shared memory: " << name << " with size: " << data_size << std::endl;
        using ShmemAllocator = ipc::allocator<double, ipc::managed_shared_memory::segment_manager>;
        using queue_t = ipc::deque<double, ShmemAllocator>; // Using deque as a queue
        const ShmemAllocator alloc_inst(managed_shm.get_segment_manager());
        managed_shm.construct<queue_t>("SharedQueue")(alloc_inst);
        managed_shm.construct<ipc::interprocess_mutex>("QueueMutex")();
    }
    catch (const boost::interprocess::interprocess_exception &e)
    {
        std::cerr << "Error creating shared memory: " << e.what() << std::endl;
...
bool SMemory::enqueue(std::string name, double value, int array_size){
    // std::cout << "[Enqueue] Testing 1st line..." << std::endl;
    if (!checkMemory(name))
    {
        std::cout << "Shared memory doesn't exist. Creating new shared memory: " << name << std::endl;
        this-> createQueue(name, array_size * sizeof(double) + sizeof(ipc::interprocess_mutex));
    }
    ipc::managed_shared_memory managed_shm{ipc::open_only, name.c_str()};
    using deque_t = ipc::deque<double, ipc::allocator<double, ipc::managed_shared_memory::segment_manager>>;
    std::pair<deque_t *, std::size_t> p = managed_shm.find<deque_t>("SharedQueue");
    if (!p.first)
    {
        std::cerr << "Error: Queue not found in shared memory!" << std::endl;
        return false;
    }
    std::pair<ipc::interprocess_mutex *, std::size_t> mutex_pair = managed_shm.find<ipc::interprocess_mutex>("QueueMutex");
    if (!mutex_pair.first)
    {
        std::cerr << "[Enqueue] Error: Mutex not found in shared memory!" << std::endl;
        return false;
    }
    ipc::scoped_lock<ipc::interprocess_mutex> lock(*mutex_pair.first); // Lock the mutex
    __sync_synchronize();
    p.first->push_back(value);                                         // Enqueue operation
    __sync_synchronize();
...
double SMemory::dequeue(std::string name)
{
    if (!checkMemory(name))
        std::cerr << "Shared Memory is not availabe for read.."<< std::endl;
    ...
    double value = p.first->front(); // Retrieve the front value
    __sync_synchronize();
    p.first->pop_front();     // Dequeue operation
    __sync_synchronize();
