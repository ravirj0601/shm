...
int main() {
    ...
    Message msg;
    while (true) {
        ssize_t result = msgrcv(msgid, &msg, sizeof(msg.mtext), 2, IPC_NOWAIT);
        if (result == -1) {
            std::cerr << "No more messages of type 2." << std::endl;
            break;
        }

        std::cout << "Process B received: " << msg.mtext << std::endl;
    }
