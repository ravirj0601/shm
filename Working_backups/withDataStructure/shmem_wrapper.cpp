...
// void SMemory::create(std::string name, unsigned int array_size)
// {
//     if (checkMemory(name))
//         return; 
//     ipc::managed_shared_memory managed_shm{ipc::create_only, name.c_str(), array_size * sizeof(double) + sizeof(ipc::interprocess_mutex)};

//     managed_shm.construct<SharedData>("SharedData")();
//     std::cout << "Created this SHM Variables:" << name << std::endl;
// }
/**
...
double SMemory::fetchVector(std::string name, unsigned int location)
{
    ...
    if (p.first)
    {
        ...
        if (mutex_pair.first)
        {
            ...
            if (location < p.first->size())
            {
                return (*p.first)[location]; // Return the value at the specified location
            }
        }
    }
